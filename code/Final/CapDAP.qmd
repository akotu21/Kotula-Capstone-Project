...
final dataset contains 1706 observations of 36 variables, although there are very few complete cases in the dataset. For my analyses, I had to make additional datasets using `group_by()` and `summarise()`. The gad_totals dataset contains the number of amphibians found on each date with the corresponding moon phase and moon illumination for each date. I used this for my first two analyses. The sp.richness.df2 dataset includes the number of species found per survey (date) with the corresponding land class for each survey. I used this for my third
analysis.
